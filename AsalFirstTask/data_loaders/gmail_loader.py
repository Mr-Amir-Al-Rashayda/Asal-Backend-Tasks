...
from .loader_base import EmailLoader # Import new base

EmailDict = Dict[str, Any]

class GmailLoader(EmailLoader): # Renamed and inherits from EmailLoader
    """
    ...
    def __init__(self, username: str, password: str, count: int = 10):
        self.username = username
        self.password = password
        self.count = count # Store count
        self.mail: Optional[imaplib.IMAP4_SSL] = None
        
    def load_emails(self) -> List[EmailDict]: # Renamed from connect_and_fetch
        """Connects to Gmail, fetches the latest emails, and returns them as EmailDict objects."""
        emails_list: List[EmailDict] = []
        ...
        try:
            ...
            # Use self.count from the constructor
            latest_ids = mail_ids[-self.count:][::-1] 
            ...
            for email_id in latest_ids:
                ...
                emails_list.append(self._build_email_record(email_id, msg))

            print(f"SUCCESS: Fetched {len(emails_list)} emails from Gmail.")
            return emails_list
